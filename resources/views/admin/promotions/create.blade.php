<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Program Promosi Baru') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8"> {{-- max-w-3xl agar form tidak terlalu lebar --}}
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:p-8 border-b border-gray-200">
                    <h3 class="text-2xl font-semibold text-emerald-700">Formulir Program Promosi</h3>
                    <p class="mt-1 text-sm text-gray-600">
                        Isi detail program promosi poin loyalitas baru di bawah ini.
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('admin.promotions.store') }}">
                    ...
                    <div class="p-6 sm:p-8 space-y-6 text-gray-900">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm">{{ old('description') }}</textarea>
                            <x-input-error :messages="$errors->get('description')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="points_awarded" :value="__('Poin yang Diberikan')" class="text-gray-700" />
                            <x-text-input id="points_awarded" class="block mt-1 w-full" type="number" name="points_awarded" :value="old('points_awarded')" required min="1" />
                        ...
                        <div class="block">
                            <label for="is_active" class="inline-flex items-center">
                                ...
                                <span class="ms-2 text-sm text-gray-700">{{ __('Aktifkan Promosi Ini Saat Disimpan') }}</span>
                            </label>
                        </div>
