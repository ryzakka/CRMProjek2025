<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Tambah Data Pasien Baru oleh Staf') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:p-8 border-b border-gray-200">
                    <h3 class="text-2xl font-semibold text-emerald-700">Formulir Pendaftaran Pasien</h3>
                    <p class="mt-1 text-sm text-gray-600">
                        Silakan isi detail pasien baru di bawah ini.
                    </p>
                </div>
                    ...
                    <div class="p-6 sm:p-8 space-y-6 text-gray-900">
                        ...
                        <div>
                            ...
                            <select name="patient_type" id="patient_type" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm">
                                ...
                                <option value="Umum" {{ old('patient_type') == 'Umum' ? 'selected' : '' }}>Umum</option>
                                <option value="BPJS Kesehatan" {{ old('patient_type') == 'BPJS Kesehatan' ? 'selected' : '' }}>BPJS Kesehatan</option>
                                <option value="Asuransi Swasta" {{ old('patient_type') == 'Asuransi Swasta' ? 'selected' : '' }}>Asuransi Swasta</option>
                                <option value="Karyawan Rekanan" {{ old('patient_type') == 'Karyawan Rekanan' ? 'selected' : '' }}>Karyawan Rekanan</option>
                                <option value="Lainnya" {{ old('patient_type') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="3" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm">{{ old('address') }}</textarea>
                            <x-input-error :messages="$errors->get('address')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="registration_date" :value="__('Tanggal Registrasi')" class="text-gray-700" />
                            <x-text-input id="registration_date" class="block mt-1 w-full" type="date" name="registration_date" :value="old('registration_date', date('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <textarea name="preferences_notes" id="preferences_notes" rows="3" class="block mt-1 w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm">{{ old('preferences_notes') }}</textarea>
